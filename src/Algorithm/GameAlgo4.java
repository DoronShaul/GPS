...
public class GameAlgo4 {
  ...
  */
	 ...
	 */
	public static int quickestPath(FruitsList fl, BoxList bl, Player player) {
		int fruIndex=0;
		...
		while(itf.hasNext()) {
			...
			if(player.playerToPoint().isDirect(temp.fruitToPoint(), bl)) {
				...
				Graph g=new Graph();
				g = GameAlgo4.buildGraph(player.playerToPoint(),
						temp.fruitToPoint(), bl);
			...
			if(currentDist<minDist) {
				minDist=currentDist;
				fruIndex=fl.flArray().indexOf(temp);
			}
		}
	...
	public static void updateBoard(ArrayList<String> board,PacmanList pl, FruitsList fl, GhostsList gl, Player player) {
		...
		for(int i=0;i<board.size();i++) {
			...
			if(splitedRow[0].equals("P")) {
				...
				player.updatePlayer(Double.parseDouble(splitedRow[2]), Double.parseDouble(splitedRow[3])); //sets the player's location to it's new location.
			}
